...
**Network-exposure blocks (`network-exposure` category)**: `tailscale serve` (mutations), `tailscale funnel`, and `tailscale up`/`set`/`login`/`logout`/`switch`/`cert` — the raw-CLI paths by which the agent's own bash could widen its network exposure below the governed dashboard/CLI seam. Putting the dashboard on the tailnet is an owner-only, SEL-audited action honouring `capabilities.tailnet_origin` (`tailnet_serve.publish`); these rules stop bash from reaching `tailscale serve` directly (bypassing `publish()` and its pin), `tailscale funnel` (which exposes a service to the **public** internet), and the node-state mutations that enable an SSH server or advertise routes. They reuse the linearized flag-run primitive and are enforced at the regex tier. A `(?!\s+status)` carve-out keeps the read subcommands (`status`, `serve status`, `funnel status`, `netcheck`, `ping`, `whois`) allowed, so the status card and any tailnet-doctor tooling still work. Kiro Crew's own publish path is unaffected: it spawns `tailscale` via `subprocess.run`, never through the `hooks.py` gate.
