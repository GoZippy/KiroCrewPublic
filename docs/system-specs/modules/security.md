...
- **Windows via WSL2** (`agent.sandbox: "wsl2"`, opt-in, never auto-selected): native Windows has no OS-level namespace primitive of its own, so this backend reuses the Linux launcher script (`_build_launcher_script` — same unshare/bind-mount logic) but executes it inside a WSL2 distribution's real Linux kernel via `wsl.exe`. Differences from the Linux path are confined to `wsl_namespace_argv()`:

  - Identity: uid/gid/home are resolved *inside* the guest (`_resolve_wsl2_identity`, a cheap `id -u && id -g && echo $HOME` round trip) rather than read from the Windows builder process, which has neither `os.getuid()` nor the guest's home. `_build_launcher_script`'s own path-joining for the guest's home-relative dirs uses `posixpath.join`, not `os.path.join` — identical behavior on real Linux (`os.path` **is** `posixpath` there), but load-bearing when the builder is Windows Python, whose `os.path.join` would embed a backslash into a script meant to run on a Linux filesystem.
  - **Masking the operator's REAL Windows-side sensitive directories, not only the guest's.** `_build_launcher_script`'s `hidden_dirs` masks the GUEST home (e.g. `/home/alice`, resolved above) — a mostly-empty account holding none of the operator's actual credentials. WSL2's default DrvFs mount makes the operator's real Windows filesystem reachable *inside* the guest at `/mnt/<drive>/...`, including `C:\Users\<user>\.aws`/`.ssh` and the governance keystone under `config_dir()` — unmasked, this defeated both the credential-hiding claim and, for the keystone, `security._SENSITIVE_HOME_DIRS`'s "the agent can neither read nor write its own ceiling" invariant (caught in review before merge). `wsl_namespace_argv()` now computes the same sensitive-directory set `_build_launcher_script` would mask natively (`_sensitive_dir_names()`, `.ssh` under the `"strict"` level to match `_build_launcher_script`'s own `HIDE_SSH`, `_relocated_policy_cache_dirs()`, `_voice_runtime_sandbox_paths()`), resolves it against the REAL Windows home, translates each entry through DrvFs, and passes the result in as `extra_hidden_dirs`. DrvFs mount verification (`_verify_wsl2_drvfs_mount`) therefore runs **unconditionally**, not only when a `cwd` is given: the masking list is only trustworthy once every path it names is confirmed reachable at the standard `/mnt/<drive>` location the translator assumes, and the function raises rather than sandbox unmasked if it cannot confirm this.
  - **`wrap_argv`/`sandboxed_spawn_argv` are shared chokepoints with dozens of callers, and `posix_shell_argv` opts IN, never opts OUT.** These two functions are used across the codebase for spawns that are overwhelmingly native-Windows executable invocations — an MCP server spawn, an `npm install`, a terminal command, a `git` call — not POSIX shell argv. An earlier revision defaulted `posix_shell_argv=True` (opt-out: only the callers known NOT to be POSIX-shaped had to say so), which a first-principles review caught by counting call sites: with that polarity, roughly nine unexamined production spawns across `cron_script.py` (MCP server spawn), `mcp_gateway/resolve_once.py`, `dashboard/handlers/{terminal_commands,files,worktree,themes,source_providers}.py`, and `git_coord.py` would ALL have silently started routing through the wsl2 guest launcher the moment an operator selected it — appending a Windows-shaped argv after a Linux launcher that expects a POSIX command, on backends that today correctly refuse or run unsandboxed. The default is now `False` (opt-IN): only the three call sites that build genuine POSIX `sh`/`bash -c` argv (`apps/lifecycle_scripts.py`, `cron_script.py`'s `run_command_sandboxed`, `hooks.py`) pass `True` explicitly. Every other caller — all nine named above, and any future one — keeps its exact pre-wsl2 behavior with zero code change of its own, which is what this PR's own scope description always claimed and what the inverted default now actually guarantees rather than merely asserts. Locked by `test_wrap_argv_posix_shell_argv_defaults_false`, which asserts the default on the real function signatures (not just the wsl2 dispatch branch) so a future edit cannot silently flip it back.
  - Staging and cleanup: the launcher script is staged inside the **guest's own filesystem** (never under Windows `config_dir()`, which is unreachable as a native WSL path) via a **single** `wsl.exe` invocation — `mkdir -p -m 700 <dir> && (set -C; cat > <path>) && chmod 700 <path>` — writing to a name generated in Python (`secrets.token_hex(16)`), not captured from `mktemp` via shell command substitution. Two things this avoids: mixing a `$(...)` substitution with piped stdin in one `wsl.exe -- sh -c` call silently drops the captured value (a WSL2 pipe/console-allocation quirk, verified live); and an earlier two-call form (create, then separately write+chmod) left an empty, discoverable file exposed to a same-UID sibling — a concurrently-sandboxed cron job or hook, exactly the untrusted workload this backend confines — for the whole gap between the two round trips. `set -C` (noclobber) additionally refuses the write outright if the target already exists, rather than silently overwriting a file a sibling pre-created. The Windows-side spawner has no path to delete a guest-native file afterwards, so the launcher is built with `unlink_self=True` and removes itself once the guest interpreter has loaded it, and the same staging round trip sweeps any sibling launcher older than `_WSL2_STALE_LAUNCHER_MINUTES` that a spawn dying before exec left behind. The window between the write and the interpreter's read is the same one the native launcher under `config_dir()/run` has: a same-UID sibling that can write there is already the operator, and nothing narrower than a second UID closes it.
  - Env: a Windows env var set on the `wsl.exe` process is **not** automatically visible inside the guest shell it starts — WSL forwards only names `WSLENV` lists. `wsl2_env_passthrough()` is a no-op unless wsl2 is selected; the two callers that inject metadata a hook/lifecycle script's shell reads by name (`hooks.py`'s `KIROCREW_HOOK_EVENT`/`KIROCREW_HOOK_CONTEXT`, `lifecycle_scripts.py`'s `NONINTERACTIVE`/`extra_env`) call it before spawning. Never applied to the platform-baseline keys (`PATH`, `HOME`, ...), which the guest must supply from its own correct values, not the Windows host's.
  - **The crew governance tree keeps its three dispositions across DrvFs.** `_build_launcher_script` derives hidden / read-only / visible for the crew data home against the GUEST home baked into `identity`, which holds none of the operator's files. `_wsl2_windows_side_masking()` runs the same derivation against the Windows home and the Windows-side `config_dir()`: the mode's sensitive dirs (which carry the crew secret leaves in every mode), the relocated governance cache and secret leaves, the voice-runtime snapshot and `.ssh` at the strict tier are hidden; the ceilings (`security_policy.json`, `admission_policy.json`, `profiles/`, `denied_commands.json`, `computer_use.json`, ...) in both spellings plus their relocated resolution and the voice-runtime parents are sealed read-only through the new `extra_readonly_dirs` seam, because an absent ceiling reads as the permissive default and hiding one would remove it. `_build_launcher_script` skips its own `config_dir()`/`Path.home()` resolutions when an `identity` is supplied (they would name Windows paths the guest cannot use) and takes both caller-supplied sets in the target filesystem's flavor instead. Coverage is therefore parity with the native backend, including the reconciliation that closed [#7332](https://github.com/kirodotdev/KiroCrew/issues/7332).
  - **`cron_script.py`'s two callers of `wrap_argv` are NOT interchangeable once wsl2 exists.** `run_command_sandboxed`'s argv (`[shell, "-c", command]`) is genuinely POSIX-shell-shaped, so it passes `posix_shell_argv=True` explicitly, and `_resolve_command_shell()` now actually probes the guest's `/bin/sh` via `_shell_is_posix_strict` (cache keyed `wsl2:<distro>:/bin/sh` — distro-qualified, since `/bin/sh` names a different binary per distro and a stale verdict from one must never vouch for another after `agent.sandbox_wsl_distro` changes) before trusting it, rather than trusting it by name — an unusual distro whose `/bin/sh` is secretly bash would otherwise reopen the exact brace-expansion bypass this vet gate exists to prevent. `run_script_sandboxed`'s argv (`[sys.executable, launcher_path]`, a native-Windows Python invocation) is NOT POSIX-shaped at all; it leaves `posix_shell_argv` at its default `False` (stated explicitly at the call site as defense-in-depth against a future default flip, even though it is redundant with the current one) — the wsl2 guest launcher appends argv verbatim after itself expecting a POSIX command, so a Windows-shaped one would neither be a valid guest path nor runnable there. Before the chokepoint's default was inverted (see above), this specific call site was the one place this was caught not opting out explicitly enough — a real regression (script crons would have silently broken the moment an operator selected wsl2), not a hardening, and the reason the default changed everywhere rather than being patched only here.

  Availability is decided empirically by `_probe_wsl2()` — the distro is registered, it has a `python3` on its PATH for the launcher to run under, and `unshare(CLONE_NEWUSER)` works inside it, each failure carrying its own remedy — mirroring `_probe_unshare`'s exact never-block-on-event-loop / retry-once-off-loop / asymmetric transient-vs-permanent cache discipline (own cache, `_wsl2_backend_ok` / `_last_wsl2_failure` — distinct from the Linux probe's, since the two answer different questions and must never let one overwrite the other's verdict). `wsl.exe` is resolved via `platform_compat.trusted_system_bin`, never a bare `PATH` lookup, and every invocation forces `WSL_UTF8=1` — verified live that WSL emits UTF-16LE otherwise, silently corrupting every string comparison downstream. A caller's working directory is threaded through as `wsl.exe --cd` (not the Windows-side `cwd=` a caller passes to `Popen`, which sets `wsl.exe`'s own launch directory, not the guest shell's) via a `/mnt/<drive>/…` translation that is verified against the guest's live mount table before being trusted (`_verify_wsl2_drvfs_mount`) rather than assumed — `wslpath` itself proved unreliable for this exact purpose on a real host (silently drops backslashes on some multi-segment paths even via a plain argv list), so this backend does not depend on it.

  **Live-verified end to end** against a real WSL2/Ubuntu host, including the isolation itself: with a *synthetic Windows home* (an isolated directory, never the operator's real one) carrying marker credential files, `.aws/credentials` and `.ssh/id_rsa` under that real Windows-side path come back "No such file or directory" from inside the sandboxed process, while an unrelated control file outside any hidden dir remains readable — proof against the actual DrvFs-reachable threat model, not merely the guest's own (separately empty) home. `WSLENV` forwarding was verified the same way: a hook-shaped spawn reads `$KIROCREW_HOOK_EVENT`/`$KIROCREW_HOOK_CONTEXT` correctly with `WSLENV` set, empty without it. The command-cron shell probe was verified against the real distro's `/bin/sh` (dash — POSIX-strict, correctly accepted). The script-cron guard was verified by spying on `wsl_namespace_argv`: with wsl2 selected and `posix_shell_argv=False`, it is never called, and the call fails closed with the identical message Windows gave for this case before wsl2 existed.

  Scope is deliberately narrow: only the POSIX-shell-shaped spawns (app lifecycle scripts, command cron jobs, script hooks) route through it; single portable-binary invocations (`git`, the AWS CLI, Papyrus's `tectonic`, Piper) are unaffected either way, since routing them through a Linux VM would buy no isolation and would need Windows↔WSL path translation for every argument.
