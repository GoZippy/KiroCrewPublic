...
- **Windows via WSL2** (`agent.sandbox: "wsl2"`, opt-in, never auto-selected): native Windows has no OS-level namespace primitive of its own, so this backend reuses the **identical** Linux launcher script (`_build_launcher_script` — same unshare/bind-mount logic, byte-for-byte) but executes it inside a WSL2 distribution's real Linux kernel via `wsl.exe`. Only three things differ from the Linux path, all confined to `wsl_namespace_argv()`: (1) uid/gid/home are resolved *inside* the guest (`_resolve_wsl2_identity`, a cheap `id -u && id -g && echo $HOME` round trip) rather than read from the Windows builder process, which has neither `os.getuid()` nor the guest's home; (2) `_build_launcher_script`'s own path-joining for the guest's home-relative dirs uses `posixpath.join`, not `os.path.join` — identical behavior on real Linux (`os.path` **is** `posixpath` there), but load-bearing when the builder is Windows Python, whose `os.path.join` would embed a backslash into a script meant to run on a Linux filesystem; (3) the launcher script is staged inside the **guest's own filesystem** (never under Windows `config_dir()`, which is unreachable as a native WSL path) via two separate `wsl.exe` invocations rather than one compound `mkdir && f=$(mktemp …) && cat > "$f"` — verified live that mixing a `$(...)` command substitution with a piped stdin in one `wsl.exe -- sh -c` call silently drops the captured value (a WSL2 pipe/console-allocation quirk, not a logic bug). Availability is decided empirically by `_probe_wsl2()`, mirroring `_probe_unshare`'s exact never-block-on-event-loop / retry-once-off-loop / asymmetric transient-vs-permanent cache discipline (own cache, `_wsl2_backend_ok` / `_last_wsl2_failure` — distinct from the Linux probe's, since the two answer different questions and must never let one overwrite the other's verdict). `wsl.exe` is resolved via `platform_compat.trusted_system_bin`, never a bare `PATH` lookup, and every invocation forces `WSL_UTF8=1` — verified live that WSL emits UTF-16LE otherwise, silently corrupting every string comparison downstream. A caller's working directory is threaded through as `wsl.exe --cd` (not the Windows-side `cwd=` a caller passes to `Popen`, which sets `wsl.exe`'s own launch directory, not the guest shell's) via a `/mnt/<drive>/…` translation that is verified against the guest's live mount table before being trusted (`_verify_wsl2_drvfs_mount`) rather than assumed — `wslpath` itself proved unreliable for this exact purpose on a real host (silently drops backslashes on some multi-segment paths even via a plain argv list), so this backend does not depend on it. **Live-verified end to end** against a real WSL2/Ubuntu host, including the isolation itself: `.aws/credentials` and `.ssh/id_rsa` are hidden while an unrelated control file outside any hidden dir remains readable, matching the Linux backend's own behavior exactly. Scope is deliberately narrow: only the POSIX-shell-shaped spawns (app lifecycle scripts, command cron jobs, script hooks) route through it; single portable-binary invocations (`git`, the AWS CLI, Papyrus's `tectonic`, Piper) are unaffected either way, since routing them through a Linux VM would buy no isolation and would need Windows↔WSL path translation for every argument.
